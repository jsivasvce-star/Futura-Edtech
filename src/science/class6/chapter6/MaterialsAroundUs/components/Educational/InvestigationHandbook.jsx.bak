...
import ancientPotteryImg from '../../../../../../assets/ancient_pottery.jpg';
    ...
    <span>
      {fullText.split('').map((char, index) => (
        <span key={index} style={{
          ...
          color: index < activeCharIndex ? '#3b2818' : 'inherit',
...
const PotterySpotlight = ({ isSoundEnabled, isActivePage, onBackToPage1, currentClue, setCurrentClue }) => {
  ...
  const playAudio = () => {
    ...
    const fullText = `${currentData.title}. ${currentData.bigFact}. ${currentData.text}`;
  ...
  useEffect(() => {
    if (isActivePage && isSoundEnabled) {
      playAudio();
    } else {
      stopAudio();
    }
  }, [currentClue, isActivePage, isSoundEnabled]);

  const timelineNode = (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: '8px', flexWrap: 'nowrap', overflow: 'hidden' }}>
      {clues.map((c, idx) => (
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, padding: '24px 32px', position: 'relative', display: 'flex', flexDirection: 'column', gap: '12px', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', alignSelf: 'flex-start' }}>
        <h2 style={{ margin: 0, fontSize: '44px', color: '#3B2A1F', fontWeight: '900', borderBottom: '4px solid #A64B27', paddingBottom: '4px' }}>
      ...
      <div style={{ display: 'flex', gap: '32px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: '1.2', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', paddingRight: '16px' }}>
          
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <h3 style={{ fontSize: '29px', fontWeight: '900', color: '#7A6A52', margin: '0' }}>
              {currentData.title}
            </h3>
          </div>

          <div style={{ background: '#FFFDF5', border: '3px solid #D9C9A3', borderRadius: '8px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px' }}>
            <h4 style={{ margin: '0', color: '#A64B27', fontSize: '24px', fontWeight: '900', letterSpacing: '1px' }}>
              DO YOU KNOW?
            </h4>
            <div style={{ fontSize: '32px', fontWeight: '900', color: '#A64B27', lineHeight: '1.1' }}>
              {currentData.bigFact}
            </div>
          </div>
          
          <div style={{ background: '#FFFFFF', border: '2px solid #D9C9A3', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
            <div style={{ fontSize: '25px', fontWeight: '600', color: '#3B2A1F', lineHeight: '1.4' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            ...
            }} style={{ background: 'transparent', color: '#3B2A1F', border: '2px solid #3B2A1F', borderRadius: '24px', padding: '10px 20px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              <ArrowLeft size={18} /> BACK
            </button>
            {currentClue < 5 && (
              <button onClick={() => setCurrentClue(prev => prev + 1)} style={{ background: '#3B2A1F', color: 'white', border: 'none', borderRadius: '24px', padding: '10px 20px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
                NEXT CLUE <ArrowRight size={18} />
              </button>
            )}
          </div>
        </div>

        {/* Right side for images / illustration */}
        <div style={{ flex: '1', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '32px' }}>
          <img src={ancientPotteryImg} alt="Ancient Pottery" style={{ width: '80%', height: 'auto', borderRadius: '16px', objectFit: 'contain', mixBlendMode: 'multiply' }} />
        </div>
      </div>
    </div>
  );
...
const InvestigationHandbookRender = ({ highestUnlockedIndex = 0, currentFlowIndex = 0, stageCompleted = false, onNext, onComplete, onBack }, ref) => {
  ...
  React.useImperativeHandle(ref, () => ({
    handleGlobalNext: () => {
      ...
      if (!isBarrier2 && !isBarrier3) {
        if (b1Page === 1) {
          setB1Page(2);
          return true;
        } else if (b1Page === 2) {
          if (currentClue === 5) {
            return false; // Tells the parent to close the handbook
          }
          return true; // Stay on the current clue if trying to proceed early
        }
      }
      return false; // Tells the parent to close the handbook
    }
  ...
  useEffect(() => {
    window.speechSynthesis.cancel();
    if (b1Page === 1 && isSoundEnabled && !isBarrier2 && !isBarrier3) {
      const text = `
        What are Objects Made Of?
        Look around you! You can see many things, like a chair, a book, a water bottle, a pencil, and so on.
        These are all objects. Even though they look different, each object is made of some material.
        Material: The substance used to make an object.
        Object: Anything we can see or use around us.
        Examples. A chair can be made of wood, plastic, or steel. A plate can be made of steel, glass, or plastic. A bottle can be made of plastic, glass, or steel.
        Think! One object can be made from different materials. One material can be used to make many different objects.
      `;
      ...
      const setVoiceAndSpeak = () => {
        const voices = window.speechSynthesis.getVoices();
        const indianVoice = voices.find(v => v.lang.includes('en-IN') && (v.name.includes('Female') || v.name.includes('Veena') || v.name.includes('Neerja') || v.name.includes('Rishi')));
        const fallbackVoice = voices.find(v => v.name.includes('Female') || v.name.includes('Google UK English Female'));
        if (indianVoice) {
  ...
  }, [b1Page, isSoundEnabled, isBarrier2, isBarrier3]);
  ...
  return (
    <div style={{
      ...
      {/* Fixed Sound Toggle Button Next to Fullscreen Button */}
      <button
        onClick={toggleSound}
        style={{
          position: 'fixed',
        ...
        title={isSoundEnabled ? "Mute Voice-over" : "Enable Voice-over"}
      >
              ...
              <div style={{ flex: 1, minHeight: 0, padding: '24px 32px', position: 'relative', display: 'flex', flexDirection: 'column', gap: '12px', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', gap: '32px', flex: 1, minHeight: 0 }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div style={{ fontSize: '23px', color: '#3B2A1F', lineHeight: '1.4', fontWeight: '600', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    ...
                    <div style={{ border: '1px dashed #A64B27', borderRadius: '8px', padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <div style={{ fontSize: '23px', fontWeight: '600', color: '#3B2A1F' }}><strong style={{ color: '#3B2A1F', fontWeight: '800', fontSize: '24px' }}>Material:</strong> The substance used to make an object.</div>
                      <div style={{ fontSize: '23px', fontWeight: '600', color: '#3B2A1F' }}><strong style={{ color: '#3B2A1F', fontWeight: '800', fontSize: '24px' }}>Object:</strong> Anything we can see or use around us.</div>
                    </div>

                    <div style={{ background: '#FFFDF5', border: '3px solid #D9C9A3', borderRadius: '8px', padding: '10px 16px' }}>
                      <h4 style={{ margin: '0 0 6px 0', color: '#A64B27', fontSize: '29px', fontWeight: '900' }}>Examples:</h4>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '23px', fontWeight: '700', color: '#3b2818' }}>
                    ...
                    <div style={{ background: '#FFFFFF', border: '2px solid #D9C9A3', borderRadius: '8px', padding: '12px 16px', display: 'flex', alignItems: 'flex-start' }}>
                      <div style={{ fontSize: '23px', fontWeight: '700', color: '#3b2818', lineHeight: '1.4' }}>
                        <strong style={{ fontWeight: '900', fontSize: '25px', color: '#A64B27' }}>Think!</strong> One object can be made from different materials. One material can be used to make many different objects.
                      </div>
                    </div>
                  </div>
                  
                  {/* Right side for images / illustration */}
                  <div style={{ width: '460px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: 'transparent' }}>
                    <img src={classroomObjectsImg} alt="Classroom Objects" style={{ width: '100%', height: 'auto', objectFit: 'contain', mixBlendMode: 'multiply', background: 'transparent' }} />
                  </div>
                </div>
              </div>
            ) : (
              // ================= PAGE 2 =================
              <PotterySpotlight isSoundEnabled={isSoundEnabled} isActivePage={b1Page === 2} onBackToPage1={() => setB1Page(1)} currentClue={currentClue} setCurrentClue={setCurrentClue} />
            )}
          </>
        ) : isBarrier2 ? (
      ...
      <div style={{
        ...
        <div style={{ color: 'var(--lesson-muted)', fontSize: '18px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
...
const InvestigationHandbook = React.forwardRef(InvestigationHandbookRender);
