      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        <svg viewBox="0 0 2000 1000" style={{ width: '100%', height: '100%', overflow: 'visible', fontFamily: 'system-ui, sans-serif' }}>
          ...
          <g style={{ transformStyle: 'preserve-3d', transform: cameraTransform, transition: 'transform 1.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
            ...
            <g style={{ opacity: stage === 'SQUASH' && substep >= 1 ? 0 : 0.8, transition: 'opacity 1s', transform: 'translate(400px, 680px)' }}>
            ...
            {blocks.map((b) => {
              ...
              if (stage === 'BUILD') {
                ...
                if (substep >= 2 && b.shell < 3) blockOpacity = 0.25; // Dim core slightly, keeping it visible
              }
              else if (stage === 'SQUASH') {
                is2D = true;
                if (b.shell < 3) {
              ...
              return (
                <g key={`${b.x}-${b.y}-${b.z}`} style={{ 
                  ...
                  <circle 
                    ...
                    stroke={dotOpacity > 0.5 ? "rgba(255,255,255,0.6)" : "none"}
                    strokeWidth="2"
                    style={{ 
                      transformOrigin: `0px ${s}px`,
          ...
          {stage === 'REVEAL' && (
            <g style={{ opacity: 1, animation: 'fadeIn 1s ease forwards' }}>
              ...
              <g transform="translate(1000, 450)">
                <circle cx="0" cy="0" r="24" fill="url(#dot-core)" filter="url(#glow)" stroke="rgba(255,255,255,0.6)" strokeWidth="2" />
              ...
              <g transform="translate(1350, 450)">
                <circle cx="0" cy="0" r="24" fill="url(#dot-shell)" filter="url(#glow)" stroke="rgba(255,255,255,0.6)" strokeWidth="2" />
                {[...Array(6)].map((_, i) => {
                   const a = i * Math.PI / 3;
                   return <circle key={i} cx={Math.cos(a)*60} cy={Math.sin(a)*60} r="24" fill="url(#dot-shell)" filter="url(#glow)" stroke="rgba(255,255,255,0.6)" strokeWidth="2" />
                })}
                <text x="0" y="220" fill="#f472b6" fontSize="40" fontWeight="bold" textAnchor="middle">7 POINTS</text>
                <g style={{opacity: substep >= 1 ? 1 : 0, transition: 'opacity 0.8s'}}>
                  <text x="0" y="280" fill="#94a3b8" fontSize="40" textAnchor="middle">↓</text>
              ...
              <g transform="translate(1700, 450)">
                ...
                {[...Array(12)].map((_, i) => {
                   ...
                   return <circle key={i} cx={Math.cos(a1)*r} cy={Math.sin(a1)*r} r="24" fill="url(#dot-shell)" filter="url(#glow)" stroke="rgba(255,255,255,0.6)" strokeWidth="2" />
                })}
