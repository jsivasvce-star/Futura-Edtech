...
# Replace LabHexagonal safely
start_idx = content.find("const LabHexagonal = () => {")
end_idx = content.find("export default function RelationsAmongSequences")

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + code + "\n" + content[end_idx:]
else:
    print("Failed to replace LabHexagonal")
    sys.exit(1)

# Fix 1: minHeight: 0 instead of 400px (so buttons don't get pushed down)
content = re.sub(
    r"<div style={{ flex: 1, minHeight: '400px'",
    r"<div style={{ flex: 1, minHeight: 0",
    content
)

# Fix 2: Flatten navigation logic in RelationsAmongSequences
content = re.sub(
    r"const \[step, setStep\] = useState\(1\); // 1: Odd, 2: Up/Down, 3: Figure it out\s+const maxSteps = 3;",
    r"const [step, setStep] = useState(1); // 1: Odd, 2: Up/Down, 3-7: Interactive Labs\n  const maxSteps = 7;",
    content
)
...
print("Applied ALL fixes atomically.")
